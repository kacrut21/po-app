...
<body class="antialiased text-gray-800" x-data="poApp()" @turbo:load.window="syncData()"
    @status-change.window="cur = $event.detail">

    <div class="flex min-h-screen">
        <!-- ─── DESKTOP SIDEBAR (md+) ─── -->
        <aside
            class="hidden md:flex flex-col w-60 bg-white border-r border-gray-100 min-h-screen sticky top-0 z-20 shadow-sm">
                ...
                <div class="flex items-center gap-2">
                    ...
                    @if(Auth::check() && Auth::user()->plan === 'starter')
                        <button @click="showUpgradeModal = true"
                            class="px-2 py-0.5 bg-gradient-to-r from-amber-400 to-orange-500 text-white text-[9px] font-extrabold rounded-md shadow-sm hover:scale-105 transition-transform tracking-wider">FREE</button>
                    @endif
                </div>
                ...
                @foreach($navItems as $item)
                    <a href="{{ route($item['route']) }}"
                        ...
                        <svg class="h-4 w-4 shrink-0 {{ request()->routeIs($item['route']) ? 'text-violet-600' : 'text-gray-400' }}"
                            fill="{{ request()->routeIs($item['route']) ? 'currentColor' : 'none' }}" viewBox="0 0 24 24"
                            stroke="currentColor" stroke-width="{{ request()->routeIs($item['route']) ? '0' : '2' }}">
                            <path stroke-linecap="round" stroke-linejoin="round" d="{{ $item['icon'] }}" />
                        </svg>
                        {{ $item['label'] }}
                        @if($item['route'] === 'pesanan')
                            <span
                                class="ml-auto text-[10px] font-bold bg-violet-100 text-violet-600 px-1.5 py-0.5 rounded-full">
            ...
            <div class="p-3 border-t border-gray-100">
                ...
                <div class="flex items-center justify-between px-2 py-2 bg-gray-50 rounded-xl">
                    <div class="flex items-center gap-2.5">
                        <div
                            class="w-8 h-8 rounded-xl bg-violet-100 flex items-center justify-center text-violet-700 text-xs font-bold uppercase">
                            {{ substr(Auth::user()->name ?? 'U', 0, 2) }}
                        </div>
                        <div class="overflow-hidden">
                            <p class="text-xs font-semibold text-gray-800 truncate">{{ Auth::user()->name ?? 'User' }}
                            </p>
        ...
        <div class="flex-1 flex flex-col min-h-screen max-w-lg mx-auto md:max-w-none w-full">
            ...
            <!-- Desktop Topbar -->
            <header
                class="hidden md:flex items-center justify-between h-14 px-6 bg-white border-b border-gray-100 sticky top-0 z-10">
    ...
    <!-- Upgrade Modal -->
    <div x-show="showUpgradeModal" style="display: none;"
        class="fixed inset-0 z-[150] flex items-center justify-center px-4" x-transition.opacity>
        <div class="absolute inset-0 bg-gray-900/40 backdrop-blur-sm" @click="showUpgradeModal = false"></div>
        <div class="bg-white rounded-2xl w-full max-w-sm relative z-10 shadow-2xl overflow-hidden"
            ...
            <!-- Header -->
            <div class="bg-gradient-to-br from-violet-600 to-purple-800 p-6 text-center relative overflow-hidden">
                <div class="absolute -top-10 -right-10 w-32 h-32 bg-white/10 rounded-full blur-2xl"></div>
                <div class="absolute -bottom-10 -left-10 w-32 h-32 bg-white/10 rounded-full blur-2xl"></div>

                <div class="relative z-10">
                    <span
                        class="inline-flex items-center justify-center px-3 py-1 rounded-full bg-white/20 text-white text-[10px] font-extrabold tracking-widest uppercase mb-3 border border-white/20">
                        Penawaran Spesial
                    </span>
                    <h3 class="text-xl font-extrabold text-white leading-tight">Upgrade ke Paket Lifetime! 🚀</h3>
                    <p class="text-violet-200 text-[12px] font-medium mt-2">Sekali bayar, nikmati akses tanpa batas
                        selamanya.</p>
                </div>

                <button @click="showUpgradeModal = false"
                    class="absolute top-3 right-3 w-8 h-8 flex items-center justify-center text-white/70 hover:text-white bg-black/10 hover:bg-black/20 rounded-full transition-colors z-20">
            ...
            <div class="p-6">
                <div class="flex items-end justify-center gap-2 mb-6">
                    <span class="text-sm font-bold text-gray-400 line-through mb-1">Rp 499.000</span>
                    <span class="text-3xl font-extrabold text-gray-900">Rp 99.000</span>
                </div>
                ...
                <div class="space-y-3 mb-8">
                    <div class="flex items-center gap-3">
                        ...
                        <span class="text-[13px] font-bold text-gray-700">Unlimited Transaksi/Pesanan</span>
                    </div>
                    <div class="flex items-center gap-3">
                        <div class="w-6 h-6 rounded-full bg-emerald-100 flex items-center justify-center shrink-0">
                            <svg class="w-3.5 h-3.5 text-emerald-600" fill="none" viewBox="0 0 24 24"
                        ...
                        <span class="text-[13px] font-bold text-gray-700">Unlimited Produk/Menu</span>
                    </div>
                    <div class="flex items-center gap-3">
                        <div class="w-6 h-6 rounded-full bg-emerald-100 flex items-center justify-center shrink-0">
                            <svg class="w-3.5 h-3.5 text-emerald-600" fill="none" viewBox="0 0 24 24"
                        ...
                        <span class="text-[13px] font-bold text-gray-700">Support Prioritas</span>
                    </div>
                    <div class="flex items-center gap-3">
                        <div class="w-6 h-6 rounded-full bg-emerald-100 flex items-center justify-center shrink-0">
                            <svg class="w-3.5 h-3.5 text-emerald-600" fill="none" viewBox="0 0 24 24"
                        ...
                        <span class="text-[13px] font-bold text-gray-700">Akses Semua Fitur Masa Depan</span>
                    </div>
                </div>

                @php
                    $waText = "Halo Admin OrderIn, saya ingin upgrade akun saya (" . (Auth::check() ? Auth::user()->email : '') . ") ke paket Lifetime Rp 99.000.";
                @endphp
                <a href="https://wa.me/6287825530343?text={{ urlencode($waText) }}" target="_blank"
                    class="w-full flex items-center justify-center gap-2 bg-[#25D366] hover:bg-[#1da851] text-white py-3.5 rounded-xl font-bold transition-colors shadow-lg shadow-green-500/30">
                    ...
                    Upgrade Sekarang via WA
                </a>
                <p class="text-[10px] text-gray-400 text-center mt-3 font-medium">Proses upgrade instan setelah
                    pembayaran terkonfirmasi.</p>
            </div>
        </div>
    </div>
